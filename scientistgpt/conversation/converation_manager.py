...
class ConversationManager:
    ...
    actions: List[ConversationNameAndAction]
    """
    ...
    def _append_and_apply_action(self, action: Action, conversation_name: Optional[str] = None):
        ...
        """
        self.actions.append(ConversationNameAndAction(conversation_name, action))
    ...
    def get_and_append_assistant_message(self, tag: Optional[str],
        ...
        indices_and_messages = self.get_conversation(conversation_name).get_chosen_messages(removed_messages)
        actual_removed_messages = removed_messages.copy()
        for index, _ in indices_and_messages:
            content = self.try_get_and_append_assistant_message(tag=tag,
                                                                agent=agent,
                                                                conversation_name=conversation_name,
                                                                comment=comment,
    ...
    def try_get_and_append_assistant_message(self, tag: Optional[str],
                                             agent: Optional[str] = None,
                                             conversation_name: Optional[str] = None,
                                             comment: Optional[str] = None,
        ...
        try:
            content = self._get_chatgpt_completion(messages)
            ...
            content = None
            action = FailedChatgptResponse(agent=agent, comment=comment, removed_messages=removed_messages, exception=e)
        except Exception:
    ...
    @staticmethod
    def _get_chatgpt_completion(messages: List[Message]) -> str:
        """
