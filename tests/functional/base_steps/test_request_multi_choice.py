...
@dataclass
class TestMultiChoiceBackgroundProductsConverser(
    TestProductsReviewGPT, MultiChoiceBackgroundProductsConverser
):
    pass
...
@pytest.mark.parametrize(
    "correct_response, incorrect_response, expected_value, error_keywords",
    [
        (
            correct_answer,
            correct_answer.replace("2", "3"),
            "2",
            ("a single character", "1", "2"),
        ),
        (
            correct_answer,
            correct_answer.replace("2", "1 and 2"),
            "2",
            ("a single character", "1", "2"),
        ),
    ],
)
def test_request_multi_choice_incorrect_then_correct(
    correct_response, incorrect_response, expected_value, error_keywords
):
...
def test_request_multi_choice_only_keeps_one_error_message():
    ...
    with LLM_SERVER_CALLER.mock(
        ["nothing", "something else", "1 and 2", "1"], record_more_if_needed=False
    ):
        assert requester.run_and_get_valid_result() == "1"

    assert (
        len(requester.conversation) == 3
    )  # 2 SYSTEM, USER prompt, correct ASSISTANT answer
    ...
    messages_lists = [h[0][0] for h in LLM_SERVER_CALLER.args_kwargs_response_history]
    ...
    assert [lst[-2].content for lst in messages_lists[1:]] == [
        "nothing",
        "something else",
        "1 and 2",
    ]
