...
from data_to_paper.research_types.hypothesis_testing.coding.original_utils import df_to_figure
...
def test_basic_plot(test_data, tmpdir):
    with run_in_directory(tmpdir):
        ...
        df_to_figure(df, filename=filename)
...
def test_plot_with_xlabel(test_data, tmpdir):
    with run_in_directory(tmpdir):
        df, filename = test_data
        df_to_figure(df, filename=filename, xlabel='X Axis Label')
        assert os.path.exists(filename)
...
def test_plot_with_ylabel(test_data, tmpdir):
    with run_in_directory(tmpdir):
        df, filename = test_data
        df_to_figure(df, filename=filename, ylabel='Y Axis Label')
        assert os.path.exists(filename)
