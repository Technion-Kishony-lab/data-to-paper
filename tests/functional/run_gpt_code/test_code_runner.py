...
def test_runner_correctly_extract_code_to_run():
    assert (
        CodeExtractor().get_modified_code_and_num_added_lines(valid_response)[0]
        == code_encoded_in_response
    )
...
def test_runner_correctly_run_extracted_code(tmpdir):
    os.chdir(tmpdir)
    result = CodeRunnerWrapper(
        code=code_encoded_in_response,
        code_runner=CodeRunner(
            allowed_open_write_files=("output.txt",), run_folder=tmpdir
        ),
...
def test_runner_raises_when_code_writes_to_wrong_file(tmpdir):
    os.chdir(tmpdir)
    _, _, _, exception = CodeRunnerWrapper(
        code=code_encoded_in_response,
        code_runner=CodeRunner(allowed_open_write_files=("wrong_output.txt",)),
    ).run_code_in_separate_process()
...
def test_runner_raise_code_timeout_exception():
    _, _, _, exception = CodeRunnerWrapper(
        timeout_sec=1, code=code_runs_more_than_1_second
    ).run_code_in_separate_process()
