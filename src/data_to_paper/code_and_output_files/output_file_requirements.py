...
class OutputFileRequirement:
    ...
    def get_code_line_label_for_file(
        self, filename: str, content: Optional[Any] = None
    ) -> Optional[str]:
        """
    ...
    def get_hyperlink_label_for_file_header(
        self, filename: str, content: Any
    ) -> Optional[str]:
        """
...
class BaseContentOutputFileRequirement(OutputFileRequirement):
    should_keep_file: bool = NotImplemented
    minimal_count: int = 1
    content_view_purpose_converter: ContentViewPurposeConverter = field(
        default_factory=ContentViewPurposeConverter
    )
    ...
    def get_issues_for_output_file_content(
        self, filename: str, content: Any
    ) -> List[RunIssue]:
        """
    ...
    def _convert_view_purpose_to_pvalue_on_str(
        self, view_purpose: ViewPurpose
    ) -> OnStr:
    ...
    def _get_block_label(
        self, filename: str, num_file: int, view_purpose: ViewPurpose
    ) -> str:
        """
    ...
    def get_header(
        ...
        return self._get_pretty_content_and_header(
            None, filename, num_file, view_purpose, header_level
        )[1]

    def get_pretty_content(
        self,
        content: Any,
        filename: str = None,
        num_file: int = 0,
        ...
        return self._get_pretty_content_and_header(
            content, filename, num_file, view_purpose
        )[0]

    def get_pretty_content_with_header(
        self,
        content: Any,
        filename: str = None,
        num_file: int = 0,
        view_purpose: ViewPurpose = ViewPurpose.PRODUCT,
        header_level: Optional[int] = 3,
    ) -> str:
        content, header = self._get_pretty_content_and_header(
            content, filename, num_file, view_purpose, header_level
        )
        if header_level:
    ...
    def _get_pretty_content_and_header(
        self,
        content: Any,
        filename: str = None,
        num_file: int = 0,
    ...
    def _get_content_and_header_for_app_html(
        ...
        content = format_text_with_code_blocks(
            content, from_md=True, is_html=True, width=None
        )
    ...
    def _get_content_and_header_for_product(
        ...
        content = self._to_str(content, filename, num_file, view_purpose)
        content = wrap_as_block(
            content, self._get_block_label(filename, num_file, view_purpose)
        )
    ...
    def _get_content_and_header_for_code_review(
        ...
        return self._get_content_and_header_for_product(
            content, filename, num_file, level, view_purpose
        )
...
class ReferencableContentOutputFileRequirement(BaseContentOutputFileRequirement):
    hypertarget_prefixes: Optional[Tuple[str]] = (
        None  # List of hypertarget prefixes to assign for each file
    )
    ...
    def _to_str(
        ...
        return self.get_formatted_text_and_header_references(
            content, filename, num_file, view_purpose
        )[0]

    def _get_prefix(self, num_file: int) -> str:
        return (
            self.hypertarget_prefixes[num_file] if self.hypertarget_prefixes else None
        )

    def get_formatted_text_and_header_references(
        self,
        content: Any,
        filename: str = None,
        num_file: int = 0,
        ...
        referencable_text = self._get_referencable_text(
            content, filename, num_file, view_purpose
        )
        return referencable_text.get_formatted_text_and_header_references(
            hypertarget_format=self._get_hyper_target_format(
                content, filename, num_file, view_purpose
            )
        )

    def _convert_content_to_labeled_text(
        self,
        content: Any,
        filename: str = None,
        num_file: int = 0,
    ...
    def _get_referencable_text(
        ...
        return self.referenceable_text_cls(
            text=self._convert_content_to_labeled_text(
                content, filename, num_file, view_purpose
            ),
            hypertarget_prefix=self._get_prefix(num_file),
        )

    def _get_hyper_target_format(
        self,
        content: Any,
        filename: str = None,
        num_file: int = 0,
        ...
        return self.content_view_purpose_converter.convert_view_purpose_to_hypertarget_format(
            view_purpose
        )
...
class TextContentOutputFileRequirement(ReferencableContentOutputFileRequirement):
    ...
    def get_issues_for_output_file_content(
        ...
        if (
            ...
            and count_number_of_tokens_in_message(content) > self.max_tokens
        ):
            ...
            issues.append(
                RunIssue(
                    ...
                    ```
                    """).format(
                        filename, extract_to_nearest_newline(content, self.max_tokens)
                    ),
...
class OutputFileRequirements(Tuple[OutputFileRequirement]):
    ...
    def _match_files_to_requirements(
        self, created_files: Iterable[str]
    ) -> Dict[str, Optional[OutputFileRequirement]]:
        """
    ...
    def get_requirements_to_output_files(
        self, created_files: Iterable[str]
    ) -> Dict[OutputFileRequirement, List[str]]:
        """
        ...
        for created_file, requirement in self._match_files_to_requirements(
            created_files
        ).items():
    ...
    def convert_to_output_file_requirements_with_content(
        self, created_files: Iterable[str], run_folder
    ) -> OutputFileRequirementsToFileToContent:
        """
        ...
        """
        requirements_to_files = self.get_requirements_to_output_files(
...
class OutputFileRequirementsToFileToContent(
    Dict[OutputFileRequirement, Dict[str, Any]]
):
    """
    ...
    def get_all_created_files(self) -> List[str]:
        ...
        return [
            filename
            for filenames_to_contents in self.values()
            for filename in filenames_to_contents.keys()
        ]
    ...
    def get_created_files_available_for_next_steps(self) -> List[str]:
        ...
        return [
            ...
            if requirement.should_keep_file
            and requirement.should_make_available_for_next_steps
        ]

    def get_created_files_to_requirements_and_contents(
        self,
        match_filename: str = "*",
        ...
        """
        return {
            filename: (requirement, content)
            for requirement, files_to_contents in self.items()
            for filename, content in files_to_contents.items()
            if fnmatch(filename, match_filename)
            and (is_content is None or is_content == (content is not None))
        }

    def get_created_content_files_to_contents(
        self, match_filename: str = "*"
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            for filename, (
                requirement,
                content,
            ) in self.get_created_files_to_requirements_and_contents(
                match_filename, is_content=True
            ).items()
        }
    ...
    def get_created_data_files(self, match_filename: str = "*") -> List[str]:
        ...
        """
        return list(
            self.get_created_files_to_requirements_and_contents(
                match_filename, is_content=False
            ).keys()
        )
    ...
    def get_created_content_files_to_pretty_contents(
        self,
        view_purpose: ViewPurpose,
        match_filename: str = "*",
        header_level: Optional[int] = 3,
    ) -> Dict[str, str]:
        """
        ...
        for requirement, files_to_contents in self.items():
            for num_file, (filename, content) in enumerate(files_to_contents.items()):
                if isinstance(
                    requirement, BaseContentOutputFileRequirement
    ...
    def get_created_content_files_and_contents_as_single_str(
        self,
        view_purpose: ViewPurpose,
        match_filename: str = "*",
        header_level: Optional[int] = 3,
