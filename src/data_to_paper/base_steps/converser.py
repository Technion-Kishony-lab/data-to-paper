...
class Converser(Copier, AppInteractor):
    ...
    model_engine: ModelEngine = field(
        default_factory=lambda: ModelEngine(
    ...
    def __post_init__(self):
        conversation_exists = (
            self.conversation_name in self.actions_and_conversations.conversations
        )
        if self.is_new_conversation is False:
            assert conversation_exists, (
                f'Conversation "{self.conversation_name}" does not exist.'
            )
        elif self.is_new_conversation is True:
            assert not conversation_exists, (
                f'Conversation "{self.conversation_name}" already exists.'
            )
        else:
    ...
    def _upon_conversation_initiation(self):
        ...
        self._app_set_focus_on_panel(
            PanelNames.RESPONSE
        )  # The Prompt panel might stay empty for a while.
        ...
        **kwargs,
        ...
        if as_action:
            self.conversation_manager.append_commenter_message(
                content=comment, tag=tag, **kwargs
            )
        else:
            print_and_log_red(comment)

    @_raise_if_reset
    def apply_get_and_append_assistant_message(
        self,
        tag: Optional[StrOrReplacer] = None,
        ...
        **kwargs,
        ...
        with self._app_temporarily_set_panel_status(
            PanelNames.RESPONSE, f"Waiting for Performer LLM ({model_engine})..."
        ):
            ...
            if send_to_app and self.app:
                self._app_send_prompt(
                    PanelNames.RESPONSE,
                    message.pretty_content(with_header=False, is_html=True),
                    provided_as_html=True,
                )
        return message

    @_raise_if_reset
    def _show_and_edit_content(
        self,
        content: StrOrReplacer,
        editing_title: str,
        editing_instructions: str,
        in_field_instructions: str,
        send_to_app: Optional[bool],
        app_panel: PanelNames,
        sleep_for: Optional[float],
    ) -> str:
        content = format_value(self, content)
        if send_to_app and self.app:
            if editing_title or editing_instructions:
                content = self._app_receive_text(
                    app_panel,
                    content,
                    title=editing_title,
    ...
    def apply_append_user_message(
        ...
        editing_title: str = None,
        editing_instructions: str = None,
        ...
        **kwargs,
        ...
        if is_edit:
            content = self._show_and_edit_content(
                content,
                editing_title,
    ...
    @_raise_if_reset
    def apply_append_system_message(
        self,
        content: StrOrReplacer,
        tag: Optional[StrOrReplacer] = None,
        comment: Optional[StrOrReplacer] = None,
    ...
    @_raise_if_reset
    def apply_append_surrogate_message(
        self,
        content: StrOrReplacer,
        tag: Optional[StrOrReplacer] = None,
        comment: Optional[StrOrReplacer] = None,
    ...
    def apply_delete_messages(
        self,
        message_designation: GeneralMessageDesignation,
        comment: Optional[str] = None,
    ):
        return self.conversation_manager.delete_messages(
            message_designation, comment=comment
        )
