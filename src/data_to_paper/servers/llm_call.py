...
from data_to_paper.env import (
    CHOSEN_APP,
    FAKE_REQUEST_HUMAN_RESPONSE_ON_PLAYBACK,
    SHOW_LLM_CONTEXT,
)
...
from .server_types import InvalidAPIKeyError, ServerErrorException

if TYPE_CHECKING:
    from data_to_paper.conversation.message import Message

TIME_LIMIT_FOR_LLM_CALL = 300  # seconds
MAX_NUM_LLM_ATTEMPTS = 5
DEFAULT_EXPECTED_TOKENS_IN_RESPONSE = 500
LLM_MAX_CONTENT_LENGTH_MESSAGE_CONTAINS = "maximum context length"  # a sub-string that indicates that an openai exception was raised due to the message content being too long
...
class TooManyTokensInMessageError(Exception):
    ...
    def __str__(self):
        return (
            f"Number of tokens in context ({self.tokens}) is too large. "
...
class LLMServerCaller(OrderedKeyToListServerCaller):
    ...
    @staticmethod
    def _get_cost_of_api_call(
        content: str, messages: List[Message], model_engine: ModelEngine
    ) -> Tuple[int, int, float]:
        """
    ...
    @staticmethod
    def _check_after_spending_money(response: litellm.ModelResponses):
        tokens_in = response["usage"]["prompt_tokens"]
        tokens_out = response["usage"]["completion_tokens"]
        cost = litellm.completion_cost(response)
        print_and_log_red(
            f"Total: {tokens_in} prompt tokens, {tokens_out} returned tokens, cost: ${cost:.2f}.",
            should_log=False,
        )

    def _log_api_usage_cost(
        self, content, messages: List[Message], model_engine: ModelEngine
    ):
    ...
    def _get_server_response(
        ...
        model_engine: Union[ModelEngine, Callable],
        **kwargs,
    ) -> Union[LLMResponse, HumanAction, Exception]:
        """
        Connect with LLM to get response to conversation.
        """
        ...
        current_model = ModelManager.get_instance().get_current_model()
        model_engine = ModelEngine(current_model)
        ...
        for attempt in range(MAX_NUM_LLM_ATTEMPTS):
            ...
            except (litellm.OpenAIError, TimeoutError) as e:
                sleep_time = 1.0 * 2**attempt
                print_and_log_red(
                    f"Unexpected LLM error:\n{type(e)}\n{e}\n"
                    f"Going to sleep for {sleep_time} seconds before trying again.",
                    should_log=False,
                )
                time.sleep(sleep_time)
                print_and_log_red(
                    f"Retrying to call LLM (attempt {attempt + 1}/{MAX_NUM_LLM_ATTEMPTS}) ...",
                    should_log=False,
                )
        else:
            raise ServerErrorException(
                ...
                response=ConnectionError(
                    f"Failed to get server response after {MAX_NUM_LLM_ATTEMPTS} attempts."
                ),
            )

        content = response["choices"][0]["message"]["content"]
        cls._check_after_spending_money(response)
...
LLM_SERVER_CALLER = LLMServerCaller()


def count_number_of_tokens_in_message(
    messages: Union[List[Message], str], model_engine: Optional[ModelEngine] = None
) -> int:
    """
...
def try_get_llm_response(
    ...
    **kwargs,
    ...
    """
    Try to get a response from LLM to a specified conversation.

    The conversation is sent to LLM after removing comment messages and any messages indicated
    in `hidden_messages`.

    If getting a response is successful then return response string.
    If failed due to LLM exception, return None.
    """
    # if model_engine is None:
    ...
    if tokens + expected_tokens_in_response > model_engine.max_tokens:
        return TooManyTokensInMessageError(
            tokens, expected_tokens_in_response, model_engine
        )
    if SHOW_LLM_CONTEXT:
        print_and_log_red(
            f"Using {model_engine} (max {model_engine.max_tokens} tokens) "
            f"for {tokens} context tokens and {expected_tokens_in_response} expected tokens."
        )
    else:
        print_and_log_red(f"Using {model_engine}.")
    try:
        action = LLM_SERVER_CALLER.get_server_response(
            messages, model_engine=model_engine, **kwargs
        )
        ...
        if LLM_MAX_CONTENT_LENGTH_MESSAGE_CONTAINS in str(e):
            return e
        else:
            raise
...
def get_human_response(app: BaseApp, **kwargs) -> HumanAction:
    ...
    response = LLM_SERVER_CALLER.get_server_response(
    ...
    if isinstance(response, LLMResponse) and CHOSEN_APP != None:  # noqa (Mutable)
        raise ValueError(
            "LLM response retrieved, instead of human action.\n"
            "Runs recorded without human actions should be replayed with the same settings\n"
            "(CHOSEN_APP = None)"
        )
...
def are_more_responses_available() -> bool:
    """
    Check if there are more recorded responses available from LLM.
    """
    return LLM_SERVER_CALLER.are_more_records_available()
