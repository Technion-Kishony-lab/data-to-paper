...
from data_to_paper.servers.model_engine import (
    LLM_CALL_PARAMETERS_NAMES,
    LLMCallParameters,
    ModelEngine,
)
...
from .message_designation import (
    GeneralMessageDesignation,
...
class ConversationManager:
    ...
    def create_conversation(self):
        self._create_and_apply_action(
            CreateConversation, participants=self.participants
        )

    def add_participants(self, agents: Iterable[Agent]):
        self._create_and_apply_action(
            AddParticipantsToConversation, participants=set(agents)
        )
    ...
    def append_message(self, message: Message, comment: Optional[str] = None, **kwargs):
        ...
        """
        self._create_and_apply_action(
            AppendMessage, message=message, comment=comment, **kwargs
        )

    def create_and_append_message(
        self,
        role: Role,
        content: str,
        tag: Optional[str],
    ...
    def append_system_message(
        ...
        **kwargs,
        ...
        self.create_and_append_message(
            Role.SYSTEM,
            content,
            tag,
            comment,
    ...
    def get_and_append_assistant_message(
        self,
        tag: Optional[str] = None,
        comment: Optional[str] = None,
        is_code: bool = False,
        ...
        **kwargs,  # for both create_message and openai params
        ...
        indices_and_messages = self.conversation.get_chosen_indices_and_messages(
            hidden_messages
        )
        actual_hidden_messages = hidden_messages.copy()

        # extract all LLM_CALL_PARAMETERS_NAMES from kwargs:
        llm_call_parameters = LLMCallParameters(
            **{k: kwargs.pop(k) for k in LLM_CALL_PARAMETERS_NAMES if k in kwargs}
        )
        ...
        model = llm_call_parameters.model_engine or ModelEngine(
            ModelManager.get_instance().get_current_model()
        )
        while True:
            message = self._try_get_and_append_llm_response(
                tag=tag,
                comment=comment,
                is_code=is_code,
                is_json=is_json,
                ...
                llm_call_parameters=llm_call_parameters,
            ...
            if model_was_bumped:
                ...
                llm_call_parameters.model_engine = model
                continue
            ...
            if len(indices_and_messages) <= 1:
                ...
                raise RuntimeError(
                    "Failed accessing openai despite removing all messages from context."
                )
            print_and_log_red(
                f"############# Removing message from context #############"
            )
            index, _ = indices_and_messages.pop(1)
            actual_hidden_messages.append(index)

    def _try_get_and_append_llm_response(
        self,
        tag: Optional[str],
        comment: Optional[str] = None,
        is_code: bool = False,
        ...
        is_json: bool = False,
        hidden_messages: GeneralMessageDesignation = None,
        llm_call_parameters: Optional[LLMCallParameters] = None,
        expected_tokens_in_response: int = None,
        **kwargs,
        ...
        """
        llm_call_parameters = llm_call_parameters or LLMCallParameters()
        if is_json:
            params = litellm.get_supported_openai_params(
                model=llm_call_parameters.model_engine
            )
            if "response_format" in params:
                llm_call_parameters.response_format = {
                    "type": "json_object",
                }
            else:
                raise ValueError(
                    f"Model {llm_call_parameters.model_engine} does not support json mode. Please set JSON_MODE = False in env file."
                )

        messages = self.conversation.get_chosen_messages(hidden_messages)
        content = try_get_llm_response(
            messages,
            expected_tokens_in_response=expected_tokens_in_response,
            **llm_call_parameters.to_dict(),
        )
        ...
        message = create_message(
            ...
            tag=tag,
            agent=self.assistant_agent,
            openai_call_parameters=None
            if llm_call_parameters.is_all_none()
            else llm_call_parameters,
            previous_code=previous_code,
            is_code=is_code,
            is_json=is_json,
        )
    ...
    def delete_messages(
        ...
        """
        self._create_and_apply_action(
            DeleteMessages, comment=comment, message_designation=message_designation
        )

    def replace_last_message(
        self, content: str, agent: Optional[Agent] = None, comment: Optional[str] = None
    ) -> Message:
        """
        ...
        self._create_and_apply_action(
            ReplaceLastMessage, comment=comment, message=message
        )
        return message
