...
from data_to_paper.servers.model_engine import LLMCallParameters, ModelEngine
...
class Message:
    ...
    openai_call_parameters: Optional[LLMCallParameters] = None
    ...
    def to_llm_dict(self):
        return {
            ...
            if self.role.is_assistant_or_surrogate()
    ...
    def pretty_repr(
        ...
        * Highlighting code blocks
        ...
        role_text = role.name + (
            f"{self.openai_call_parameters}" if self.openai_call_parameters else ""
        )
        if role == Role.SYSTEM:
            role_model_agent_conversation_tag = (
                f'{role_text} casting {agent_text} -> "{conversation_name}" '
            )
        else:
            role_model_agent_conversation_tag = (
                f'{role_text}{agent_text} -> "{conversation_name}" {tag_text} '
            )

        if role == Role.COMMENTER:
            return colored_text(
                num_text + role_model_agent_conversation_tag + ": " + content,
                text_color,
            )
        ...
        if abbreviate_content:
            ...
            return colored_text(
                num_text + role_model_agent_conversation_tag + ": \n" + content,
                text_color,
            )
    ...
    def get_content_after_hiding_incomplete_code(self) -> (str, bool):
        ...
        if len(formatted_sections) == 0:
            ...
            is_incomplete_code = (
                ...
                and not last_section.is_complete
            )
    ...
    def _get_triple_quote_formatted_content(
        self, with_header: bool = True
    ) -> (str, bool):
    ...
    def pretty_content(
        ...
        """
        content, is_incomplete_code = self._get_triple_quote_formatted_content(
            with_header
        )
    ...
    def get_short_description(
        self, left: int = 85, right: int = -20, model: ModelEngine = None
    ) -> str:
        return (
            f"{self.role.name:>9} ({self.get_number_of_tokens(model):>4} tokens): "
...
class CodeMessage(Message):
    ...
    def get_code_diff(self) -> str:
        ...
        diff = difflib.unified_diff(
            self.previous_code.strip().splitlines(),
    ...
    def _make_content_for_pretty(self, content: str, is_incomplete_block: bool):
        ...
        if self.extracted_code and not is_incomplete_block and self.previous_code:
            ...
            if not is_lower_eq(
                line_count(self.extracted_code) - line_count(diff),
                MINIMAL_COMPACTION_TO_SHOW_CODE_DIFF,
            ):
...
def create_message(
    ...
    agent: Optional[Agent] = None,
    ignore: bool = False,
    openai_call_parameters: LLMCallParameters = None,
    context: List[Message] = None,
    previous_code: str = None,
    is_code: bool = False,
    is_json: bool = False,
...
def create_message_from_other_message(
    other_message: Message, content: Optional[str] = None, agent: Optional[Agent] = None
) -> Message:
    return create_message(
        role=other_message.role,
        ...
        previous_code=other_message.previous_code
        if isinstance(other_message, CodeMessage)
        else None,
